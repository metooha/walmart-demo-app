...
export type ButtonAnchorProps = ButtonBaseProps &
  Omit<React.ComponentPropsWithoutRef<'a'>, 'className' | 'style' | 'children'> & {
    /**
...
export type ButtonButtonProps = ButtonBaseProps &
  Omit<React.ComponentPropsWithoutRef<'button'>, 'className' | 'style' | 'children'> & {
    /**
...
/**
 * buttonVariants - Simple utility for backward compatibility
 * For new code, always use the Button component directly with LD 3.5 variants.
 */
const variantClasses: Record<string, string> = {
  default: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold rounded-full text-sm',
  primary: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold text-white rounded-full text-sm',
  secondary: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold border rounded-full text-sm bg-white',
  destructive: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold text-white rounded-full text-sm',
  outline: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold border rounded-md text-sm',
  ghost: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold rounded-md text-sm',
  link: 'inline-flex items-center justify-center gap-2 whitespace-nowrap font-bold text-sm underline-offset-4 hover:underline',
};
...
export function buttonVariants(opts?: { variant?: string; size?: string; className?: string }): string {
  const v = opts?.variant || 'default';
  const s = opts?.size || 'default';
  return [variantClasses[v] || variantClasses.default, sizeClasses[s] || sizeClasses.default, opts?.className].filter(Boolean).join(' ');
}
